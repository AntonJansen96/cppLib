...
template <typename Container> auto slice(Container const &container, size_t m, size_t n)
{
...
template <typename Container> auto view(Container const &container, size_t m, size_t n)
{
    auto a = std::next(container.begin(), m);
    auto b = std::next(container.begin(), n);
...
inline auto range(long start, long stop = 0, long step = 1)
{
    ...
    return std::ranges::views::iota(0, step > 0 ? (stop - start + step - 1) / step
